"""
GRU Forecaster Module
=====================

GRU-based neural network for groundwater quality forecasting.
Includes categorical embeddings for location features.
"""
...
if TORCH_AVAILABLE:
    class FeatureAttention(nn.Module):
        ...
        Applied after the RNN hidden state: the model learns which
        feature dimensions are most predictive per sample, rather
        than treating all hidden units equally.
        """
        ...
        def forward(self, x: torch.Tensor) -> torch.Tensor:
            # x: (batch, hidden_size) — treat each hidden unit as a "token"
            x3d = x.unsqueeze(1)                                   # (B, 1, H)
            q = self.query(x3d)
            k = self.key(x3d)
            v = self.value(x3d)
            attn = torch.softmax(torch.bmm(q, k.transpose(1, 2)) * self.scale, dim=-1)
            out = torch.bmm(attn, v).squeeze(1)                    # (B, H)
            return out + x                                          # residual connection

    class GRUNetwork(nn.Module):
        """GRU-based network with categorical embeddings and feature attention."""
        ...
        def __init__(
            ...
            for idx, cardinality in categorical_cardinalities.items():
                ...
                self.embeddings[str(idx)] = nn.Embedding(cardinality + 1, emb_dim)  # +1 for unknown
                total_embedding_dim += emb_dim

            # Input size = numeric features + embedding dimensions
            input_size = n_numeric + total_embedding_dim

            # GRU layer (treating single time step as sequence of 1)
            self.gru = nn.GRU(
...
class GRUForecaster(BaseForecaster):
    """
    GRU-based forecaster for groundwater quality.
    ...
    def fit(
        ...
        try:
            ...
            logger.warning("Failed to move GRU network to %s — falling back to CPU", self._device)
        ...
        try:
            ...
            if use_focal:
                ...
                logger.info(f"GRU using FocalLoss(gamma={focal_gamma})")
            else:
        ...
        logger.info(f"GRU fitted with {self._n_features} features, {self._n_classes} classes")
    ...
    def load(self, path: Union[str, Path]) -> 'GRUForecaster':
